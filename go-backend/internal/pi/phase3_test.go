...
func TestTaskManagerCreateWithID(t *testing.T) {
	tm := NewTaskManager()
	...
	if task.ID != "custom-id" {
		t.Errorf("ID = %q, want %q", task.ID, "custom-id")
	}
}
...
func TestTaskManagerGet(t *testing.T) {
	...
	task, _ := tm.Create(Task{Title: "test"})
...
func TestTaskManagerGetNotFound(t *testing.T) {
	...
	if err == nil {
		t.Error("Get should error for nonexistent task")
	}
}
...
func TestTaskManagerUpdate(t *testing.T) {
	...
	updated, err := tm.Update(task.ID, TaskUpdate{
		Status: statusPtr(TaskCompleted),
		Output: &output,
	})
	if err != nil {
	...
	if updated.Output != "completed successfully" {
		t.Errorf("Output = %q", updated.Output)
	}
}
...
func TestTaskManagerStop(t *testing.T) {
	...
	task, _ := tm.Create(Task{Title: "test"})
	stopped, err := tm.Stop(task.ID, "user cancelled")
	if err != nil {
		t.Fatalf("Stop failed: %v", err)
	}
	if stopped.Status != TaskFailed {
		t.Errorf("Status = %q, want %q", stopped.Status, TaskFailed)
	}
	if stopped.Error != "user cancelled" {
		t.Errorf("Error = %q, want %q", stopped.Error, "user cancelled")
	}
}
...
func TestTaskManagerDelete(t *testing.T) {
	...
	task, _ := tm.Create(Task{Title: "test"})
	if err := tm.Delete(task.ID); err != nil {
		t.Fatalf("Delete failed: %v", err)
	}

	if _, err := tm.Get(task.ID); err == nil {
		t.Error("Get should fail after delete")
	}
}

func TestTaskManagerListByProject(t *testing.T) {
	tm := NewTaskManager()

	tm.Create(Task{Title: "task1", ProjectDir: "/project-a"})
	tm.Create(Task{Title: "task2", ProjectDir: "/project-a"})
	tm.Create(Task{Title: "task3", ProjectDir: "/project-b"})

	tasks := tm.ListByProject("/project-a")
	if len(tasks) != 2 {
		t.Errorf("ListByProject = %d tasks, want 2", len(tasks))
	}

	all := tm.ListByProject("")
	if len(all) != 3 {
		t.Errorf("ListByProject('') = %d tasks, want 3", len(all))
	}
}

func TestTaskManagerListByAgent(t *testing.T) {
	tm := NewTaskManager()

	tm.Create(Task{Title: "task1", ParentAgent: "agent-1"})
	tm.Create(Task{Title: "task2", ParentAgent: "agent-2"})

	tasks := tm.ListByAgent("agent-1")
	if len(tasks) != 1 {
		t.Errorf("ListByAgent = %d tasks, want 1", len(tasks))
	}
}

func TestTaskManagerDependencies(t *testing.T) {
	tm := NewTaskManager()

	t1, _ := tm.Create(Task{Title: "task1"})
	t2, _ := tm.Create(Task{Title: "task2"})

	err := tm.SetDependencies(t2.ID, []string{t1.ID}, nil)
	if err != nil {
	...
	if len(got.BlockedBy) != 0 {
		// We set blocks, not blockedBy here
	}
	if len(got.Blocks) != 1 || got.Blocks[0] != t1.ID {
		t.Errorf("Blocks = %v, want [%s]", got.Blocks, t1.ID)
	}
}

func TestTaskManagerCanStart(t *testing.T) {
	tm := NewTaskManager()
	...
	// t2 blocked by t1
	tm.SetDependencies(t2.ID, nil, []string{t1.ID})

	// t2 cannot start because t1 is not complete
	can, _ := tm.CanStart(t2.ID)
	if can {
		t.Error("t2 should not be able to start while t1 is pending")
	}

	// Complete t1
	tm.Update(t1.ID, TaskUpdate{Status: statusPtr(TaskCompleted)})

	// Now t2 can start
	can, _ = tm.CanStart(t2.ID)
	if !can {
		t.Error("t2 should be able to start after t1 is complete")
	}
}

func TestTaskManagerNoCycles(t *testing.T) {
	tm := NewTaskManager()

	t1, _ := tm.Create(Task{Title: "task1"})
	t2, _ := tm.Create(Task{Title: "task2"})
	t3, _ := tm.Create(Task{Title: "task3"})

	// Create cycle: t1 → t2 → t3 → t1
	tm.SetDependencies(t2.ID, nil, []string{t1.ID})
	tm.SetDependencies(t3.ID, nil, []string{t2.ID})

	err := tm.SetDependencies(t1.ID, nil, []string{t3.ID})
	if err == nil {
		t.Error("SetDependencies should detect cycle")
	}
}
...
func TestTaskManagerShutdown(t *testing.T) {
	...
	tm.Create(Task{Title: "task1"})
	tm.Shutdown()

	tasks := tm.ListByProject("")
	if len(tasks) != 0 {
		t.Errorf("Shutdown should clear all tasks, got %d", len(tasks))
	}
}
...
func TestTaskIsTerminal(t *testing.T) {
	...
	for _, tt := range tests {
		got := TaskIsTerminal(tt.status)
		if got != tt.terminal {
			t.Errorf("TaskIsTerminal(%q) = %v, want %v", tt.status, got, tt.terminal)
		}
	}
}
