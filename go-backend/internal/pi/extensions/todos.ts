/**
 * Todos & Scratch Pad Extension
 *
 * Registers native Pi tools for task tracking and note-taking:
 *   - write_todos: Save/update a todo list (persists as artifact via API)
 *   - read_todos: Load the current todo list
 *   - write_scratch_pad: Save observations and notes
 *   - read_scratch_pad: Load scratch pad notes
 *
 * Todos use the orchestrator's artifacts API for persistence.
 */
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		name: "write_todos",
		label: "Write Todos",
		description: "Save or update the todo list. Use checkbox format: - [ ] for pending, - [>] for in-progress, - [x] for done.",
		parameters: Type.Object({
			todos: Type.String({ description: "Todo list in markdown checkbox format. Each line: '- [ ] task', '- [>] task', or '- [x] task'." }),
		}),

		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			try {
				// Parse todos for details
				const lines = params.todos.split("\n").filter((l) => l.trim().startsWith("- ["));
				const todos: Todo[] = [];
		...
		renderResult(result, _opts, theme, _ctx) {
			...
			return new Text(
				theme.fg("success", "✓ ") +
				theme.fg("muted", `${done}/${total} todos saved`),
				0,
				0,
			);
		},
	...
	pi.registerTool({
		...
		async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
			try {
				const raw = callApi("/api/pi/artifacts?agentId=default");
	...
	pi.registerTool({
		...
		renderResult(result, _opts, theme) {
			...
			return new Text(
				theme.fg("muted", `${lines} lines in scratch pad`),
				0,
				0,
			);
		},
