...
func buildComputerUseSubAgentPrompt(cfg SubAgentPromptConfig) string {
	...
	prompt := strings.Join([]string{
		...
		"",
		"Important: The bash tool runs on the HOST, NOT in the sandbox container.",
		"To run commands INSIDE the sandbox container, use the computer_use_exec tool.",
		"Example: computer_use_exec to install packages: {\"command\": \"apt update && apt install -y cowsay\"}",
		"Example: computer_use_exec to download files: {\"command\": \"curl -o /sandbox/workspace/cat.png URL\"}",
		"Example: computer_use_exec to verify: {\"command\": \"ls -la /sandbox/workspace/cat.png\"}",
		"Use `apt update && apt install -y PACKAGE` to install packages inside the sandbox.",
		"You are running as root inside the sandbox. Do NOT use sudo.",
		"",
		"## Credentials",
		"",
