...
func BuildOrchestratorPrompt(tools []core.Tool, sandboxID string, projectContext string, examples string) string {
	...
	b.WriteString("To analyze a local screenshot, get its data URI first: `curl -s http://localhost:9876/file/SCREENSHOT_PATH | python3 -c \"import sys,json; print(json.load(sys.stdin)['data_uri'])\"`\n")
	b.WriteString("Then pass the data URI to `analyze_image` as the imageSource parameter.\n")
	b.WriteString("For remote image URLs (from extract_images), pass the URL directly to `analyze_image`.\n")
