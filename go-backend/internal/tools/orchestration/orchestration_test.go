...
func TestResolveRole_KernelRole(t *testing.T) {
	...
	instructions, tools, _, _, model, division, _, _ := resolveRole("researcher", nil, 15, 0.4, webMCPServer, nil)
...
func TestResolveRole_OrgRoleOverridesKernel(t *testing.T) {
	...
	instructions, tools, rounds, temp, model, division, _, _ := resolveRole("sarah", nil, 15, 0.4, nil, roleMap)
...
func TestResolveRole_DivisionHead(t *testing.T) {
	...
	instructions, _, _, _, model, division, _, _ := resolveRole("research-director", nil, 15, 0.4, nil, roleMap)
...
func TestResolveRole_CustomInstructions(t *testing.T) {
	...
	instructions, tools, rounds, temp, model, division, _, _ := resolveRole(customInstructions, []string{"bash", "grep"}, 20, 0.6, nil, nil)
...
func TestResolveRole_ExplicitToolsOverrideRole(t *testing.T) {
	...
	_, tools, _, _, _, _, _, _ := resolveRole("alex", []string{"bash", "read_file", "write_file"}, 15, 0.4, nil, roleMap)
...
func TestResolveRole_MCPExpansion(t *testing.T) {
	...
	_, tools, _, _, _, _, _, _ := resolveRole("scout", nil, 15, 0.4, webMCPServer, roleMap)
...
func TestResolveRole_DefaultOverrides(t *testing.T) {
	...
	_, _, rounds, temp, _, _, _, _ := resolveRole("custom", nil, 15, 0.4, nil, roleMap)
	if rounds != 25 {
	...
	_, _, rounds2, temp2, _, _, _, _ := resolveRole("custom", nil, 30, 0.7, nil, roleMap)
	if rounds2 != 30 {
