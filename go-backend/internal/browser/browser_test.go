...
func TestNewBrowserClientEmptyURL(t *testing.T) {
	_, err := NewBrowserClient("", zap.NewNop())
	if err == nil {
		t.Error("expected error for empty URL")
	}
}

func TestNewBrowserClientSuccess(t *testing.T) {
	client, err := NewBrowserClient("ws://localhost:3000", zap.NewNop())
	if err != nil {
...
// ── BrowserClient session management ──────────────────────────

func TestBrowserClientGetStateNoSession(t *testing.T) {
	client, _ := NewBrowserClient("ws://localhost:3000", zap.NewNop())
...
func TestBrowserClientGetScreenshotNoSession(t *testing.T) {
	client, _ := NewBrowserClient("ws://localhost:3000", zap.NewNop())
	_, err := client.GetScreenshot("nonexistent")
	if err == nil {
		t.Error("expected error for missing session")
	}
}

func TestBrowserClientCloseSessionNoSession(t *testing.T) {
	client, _ := NewBrowserClient("ws://localhost:3000", zap.NewNop())
	// Closing nonexistent session should not error (idempotent)
	if err := client.CloseSession("nonexistent"); err != nil {
		t.Errorf("unexpected error: %v", err)
	}
}

func TestBrowserClientShutdown(t *testing.T) {
	client, _ := NewBrowserClient("ws://localhost:3000", zap.NewNop())
	// Should not panic
	client.Shutdown()
}

func TestBrowserClientSelectorForElement(t *testing.T) {
	client, _ := NewBrowserClient("ws://localhost:3000", zap.NewNop())
	...
	client.sessions["test"] = sess

	selector := client.selectorForElement(sess, 1)
	if selector != "#btn" {
		t.Errorf("expected #btn, got %q", selector)
	}

	selector = client.selectorForElement(sess, 99)
	if selector != "" {
		t.Errorf("expected empty for missing element, got %q", selector)
	}
}
