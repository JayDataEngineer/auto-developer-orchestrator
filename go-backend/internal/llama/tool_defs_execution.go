...
func init() {
	allTools = append(allTools,
		...
		ToolSpec{
			Name:             "file_write",
			Category:         CategoryExecution,
			Description:      "create or overwrite a file (cannot overwrite without overwrite:true)",
			Schema:           `{"file_path": "/sandbox/workspace/newfile.go", "content": "package main\nfunc main() {}"}`,
			Returns:          "Creates parent dirs automatically. Refuses to overwrite existing files unless overwrite:true is set. Returns diff when overwriting.",
			ParametersSchema: `{"type":"object","properties":{"file_path":{"type":"string","description":"Absolute path in sandbox"},"content":{"type":"string","description":"File content to write"},"overwrite":{"type":"boolean","description":"Allow overwriting existing files (default: false). Returns diff of changes."}},"required":["file_path","content"]}`,
		},
		ToolSpec{
			Name:             "file_edit",
			Category:         CategoryExecution,
			...
			Returns:          "Finds and replaces old_string with new_string. Returns a diff showing what changed. Set replace_all for multiple matches. File must be read first. Use undo_edit to revert if needed.",
		...
		ToolSpec{
			Name:             "undo_edit",
			Category:         CategoryExecution,
			Description:      "undo the last edit to a file, restoring previous content",
			Schema:           `{"file_path": "/sandbox/workspace/main.go"}`,
			Returns:          "Restores the file to its state before the last file_edit. Keeps up to 10 snapshots per file.",
			ParametersSchema: `{"type":"object","properties":{"file_path":{"type":"string","description":"Absolute path of file to undo edit on"}},"required":["file_path"]}`,
		},
		ToolSpec{
			Name:             "file_grep",
			Category:         CategoryExecution,
