...
def load_cookies() -> list[dict]:
    ...
    Run `python3 plugins/twitter-automation/skills/twitter-automation/scripts/twitter_session.py --cookies-from-browser brave` first
...
def twitter_session(headless: bool = True, wait_seconds: int = 3) -> Generator:
    ...
    if not cookies:
        ...
        raise RuntimeError(
            ...
            f"Run: python3 plugins/twitter-automation/skills/twitter-automation/scripts/twitter_session.py --cookies-from-browser brave"
        )
    ...
    with SB(uc=True, headless=headless) as sb:
        ...
        if not any(c["name"] == "auth_token" for c in cookies if c.get("name") == "auth_token"):
            raise RuntimeError(
                ...
                "Re-run: python3 plugins/twitter-automation/skills/twitter-automation/scripts/twitter_session.py --cookies-from-browser brave"
            )
