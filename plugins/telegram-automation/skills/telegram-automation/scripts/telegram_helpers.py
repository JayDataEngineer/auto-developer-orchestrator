...
`python3 plugins/telegram-automation/skills/telegram-automation/scripts/telegram_session.py --bootstrap` and reused forever.
...
Setup checklist (one-time):
  ...
  2. python3 plugins/telegram-automation/skills/telegram-automation/scripts/telegram_session.py --setup-credentials API_ID API_HASH +PHONE
  3. python3 plugins/telegram-automation/skills/telegram-automation/scripts/telegram_session.py --bootstrap
  4. Verify: python3 plugins/telegram-automation/skills/telegram-automation/scripts/telegram_session.py --check  → valid: true
"""
import json
import os
...
def load_credentials() -> dict:
    ...
    if p is None or not p.exists():
        raise RuntimeError(
            ...
            f"Run: python3 plugins/telegram-automation/skills/telegram-automation/scripts/telegram_session.py --setup-credentials API_ID API_HASH PHONE"
        )
    with open(p) as f:
        data = json.load(f)
...
def telegram_session():
    ...
    if not has_valid_session():
        ...
        raise RuntimeError(
            ...
            f"Run: python3 plugins/telegram-automation/skills/telegram-automation/scripts/telegram_session.py --bootstrap"
        )
    ...
    try:
        if not client.is_user_authorized():
            raise RuntimeError(
                ...
                "Re-run: python3 plugins/telegram-automation/skills/telegram-automation/scripts/telegram_session.py --bootstrap"
            )
        yield client
    finally:
        client.disconnect()
