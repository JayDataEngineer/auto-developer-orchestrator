...
All functions raise `RuntimeError` with a clear message if the session is missing or invalid. Run `python3 plugins/telegram-automation/skills/telegram-automation/scripts/telegram_session.py --check` to diagnose.

## Setup (one-time)

```bash
python3 plugins/telegram-automation/skills/telegram-automation/scripts/telegram_session.py --setup-credentials API_ID API_HASH +PHONE
python3 plugins/telegram-automation/skills/telegram-automation/scripts/telegram_session.py --bootstrap     # interactive SMS auth
python3 plugins/telegram-automation/skills/telegram-automation/scripts/telegram_session.py --check         # verify
```
