...
```bash
python3 plugins/game-studio-workflows/skills/game-studio-workflows/scripts/forge_client.py health
```
...
```bash
python3 plugins/game-studio-workflows/skills/game-studio-workflows/scripts/forge_client.py generate \
    "isometric 2.5D cyberpunk character, teal and crimson palette, neutral background" \
    --mode image \
    --out art/cycle-1/char_01.png
```
...
python3 plugins/game-studio-workflows/skills/game-studio-workflows/scripts/forge_client.py generate \
    ...
    --out art/cycle-1/tile_asphalt.png
```
...
| Generation returned but file missing | Save the response JSON to `art/failures/` and continue |
| NSFW / inappropriate output | Discard, log to `art/rejected.json`, try a stricter prompt |
