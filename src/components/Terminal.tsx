...
export const Terminal: React.FC<TerminalProps> = ({ logs = [], logEndRef, onRetry, onCommand }) => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-black border border-border overflow-hidden min-h-[300px] lg:min-h-0 relative group">
      <div className="h-10 bg-secondary border-b border-border flex items-center justify-between px-6 shrink-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="ml-4 flex items-center gap-2">
            ...
            <span className="text-sm font-bold tracking-[0.25em] text-zinc-500 uppercase font-mono">SYS_CORE_BASH</span>
          </div>
        </div>
        <div className="flex items-center gap-4">
          {hasFailure && onRetry && (
            <button 
              onClick={onRetry}
              className="px-3 py-1 bg-red-500/10 border border-red-500/30 text-red-500 text-sm font-bold hover:bg-red-500/20 transition-all flex items-center gap-2 glow-primary"
            >
              <Zap size={12} />
              RETRY_SEQUENCE
            </button>
      ...
      <div className="flex-1 overflow-y-auto p-6 font-mono text-xs leading-relaxed text-zinc-400 terminal-scrollbar select-text bg-black">
        {logs.map((log, i) => (
      ...
      <form onSubmit={handleCommandSubmit} className="p-3 bg-black border-t border-border flex items-center gap-3 shrink-0">
        <div className="flex items-center gap-2 px-3 py-1 bg-secondary border border-border text-xs font-bold text-primary uppercase tracking-widest font-mono">
          $
        </div>
        <div className="relative flex-1 flex items-center">
          <input 
            ...
            className="flex-1 bg-transparent border-none outline-none text-sm font-mono text-white placeholder:text-zinc-900 z-10"
          />
          {command.length === 0 && (
            <div className="absolute left-0 w-2.5 h-4 bg-primary animate-pulse ml-0.5" />
          )}
        </div>
        <div className="hidden md:flex items-center gap-2 px-3 py-1 bg-secondary border border-border text-xs font-bold text-zinc-700 uppercase tracking-[0.3em]">
          SYS_READY_TTY1
        </div>
      </form>
    </div>
