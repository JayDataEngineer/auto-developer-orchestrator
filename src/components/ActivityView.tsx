...
import { 
  ...
  Terminal as TerminalIcon, Cpu, Zap, Radio
} from 'lucide-react';
...
export const ActivityView: React.FC<ActivityViewProps> = ({ logs }) => {
  ...
  const activities = logs.slice().reverse().map((log, index) => {
    ...
    return { id: index, message, time, icon, color, bg, border };
  });
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/5 pb-12">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              { label: "Active Procs", value: activities.length + 12, icon: <Cpu size={14} /> }
            ].map(stat => (
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-zinc-300 font-mono mt-0.5 max-w-2xl leading-relaxed">
                        {act.message.split(':').slice(1).join(':') || act.message}
                      </div>
                    </div>
                  </div>
