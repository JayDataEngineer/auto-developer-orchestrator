  ...
  return (
    ...
    >
      <div className="absolute top-0 right-0 p-8">
        <div className="text-sm font-bold tracking-[0.3em] text-primary bg-primary/5 px-4 py-2 border border-primary/30 uppercase font-mono">
      ...
      <div className="grid grid-cols-3 gap-4 lg:gap-12 pt-10 border-t border-border">
        <div className="flex flex-col gap-2">
          <span className="block text-xs uppercase font-bold tracking-[0.2em] text-zinc-600">Neural Agent</span>
          <span className="text-xs lg:text-sm font-mono text-white font-bold uppercase tracking-tight">Pi-Agent</span>
        </div>
        <div className="flex flex-col gap-2">
          <span className="block text-xs uppercase font-bold tracking-[0.2em] text-zinc-600">Active Buffer</span>
          <span className="text-xs lg:text-sm font-mono text-white truncate block font-bold">server.ts</span>
        </div>
        <div className="flex flex-col gap-2">
          <span className="block text-xs uppercase font-bold tracking-[0.2em] text-zinc-600">Processing Time</span>
        ...
        <div className="mt-10 pt-8 border-t border-white/5 flex flex-col sm:flex-row gap-4">
          {task.status === 'in-progress' ? (
            <>
              <button 
                onClick={onReview}
                className="flex-[2] glass border border-primary/40 text-primary font-bold py-5 rounded-2xl text-xs uppercase tracking-[0.2em] hover:bg-primary/5 transition-all flex items-center justify-center gap-3 glow-primary hover:scale-105 active:scale-95"
              >
                <ExternalLink size={18} />
                Deep Review & Pivot
              </button>
              <button className="flex-1 glass-dark border border-white/10 rounded-2xl text-xs font-bold uppercase tracking-[0.2em] hover:bg-white/5 transition-all text-zinc-400 hover:text-white hover:border-white/20">
                Cancel
              </button>
            </>
          ) : (
            <button 
              onClick={onDispatch}
              className="flex-1 bg-primary text-black font-black py-6 text-sm uppercase tracking-[0.4em] hover:bg-primary/90 transition-all flex items-center justify-center gap-4 glow-primary shadow-[0_0_25px_rgba(255,0,255,0.4)] active:scale-95"
            >
