...
def _media_content_block(kind: str, b64: str, mime: str) -> dict:
    """The multimodal content block for ``kind``. Image/audio use the native
    deepagents ContentBlock; video uses a hand-rolled ``video_url`` data-URI
    (langchain-openai has no native video path). Raises ``ValueError`` on
    unsupported ``kind``.
    """
...
def _invoke_primary_media(
    model: Any, b64: str, mime: str, prompt: str | None, kind: str,
) -> str:
...
def _onnx_describe(
    ...
    parts += ["--image", shlex.quote(image_path)] if image_path else ["--image-url", shlex.quote(image_url or "")]
    if prompt:
