...
def _skills_dirs(org: str | None = None) -> list[Path]:
    ...
    With an ``org``: that org's ``skills/`` wins, then ``profiles/_shared/skills``,
    ...
    Scans both ``profiles/`` and ``profiles/specialists/`` for org skills dirs."""
    orgs = project_root() / "profiles"
    roots: list[Path] = []
    if org:
