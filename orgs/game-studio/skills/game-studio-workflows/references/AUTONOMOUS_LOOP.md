...
delegate_async(
    ...
    task=f"Cycle {n} art: <art goal from plan>. Write assets to /sandbox/workspace/art/cycle-{n}/. Use FORGE_WORKFLOW skill. Save manifest to art/cycle-{n}/manifest.json."
)
delegate_async(
    role="gameplay_programmer",
    task=f"Cycle {n} integration: <gameplay goal>. Save scene changes. Use GODOT_VIA_MCP skill if bridge is up; godot CLI otherwise. Update /sandbox/workspace/game/cycle-{n}/changelog.md."
)
# 2. Wait
results = collect_results()
if any(r.failed for r in results):
    log failure, decide retry vs abort

# 3. QA
delegate_to(
    role="qa_tester",
    task=f"QA cycle {n}. Screenshot the viewport via godot_client (or godot CLI headless fallback). Write vibe.json per MEDIA_QA skill. Output: /sandbox/workspace/qa/cycle-{n}/vibe.json."
)
...
python3 /sandbox/surreal_client.py complete-task \
    --id "$TASK_ID" \
    --delegated-to "technical_artist" "gameplay_programmer" "qa_tester" \
...
| Failure | Recovery |
|---------|----------|
| delegate_async to technical_artist times out | Skip art cycle, gameplay_programmer runs with existing assets |
| collect_results empty after 60s | Retry collect_results once, then assume both failed |
| qa_tester can't screenshot (Godot down + CLI failing) | Skip QA, log "qa_skipped: godot unreachable", continue |
...
```markdown
## Cycle N (timestamp)
- delegated to: technical_artist (art/char_02.png), gameplay_programmer (player.gd +12 -3)
