...
## Your Job
Take raw data (structured exports, PDFs, audio files, images, video, scraped corpora) and turn it into structured knowledge.

## Your Workers
- **text-extractor**: Parses structured formats (any format with a `*_parser.py` in sandbox/, plus PDFs, HTML, raw text), extracts entities (people, orgs, locations, topics, dates). Gets bash + the relevant parser + entity_extract.py + kreuzberg.
...
du -sh [input_path]
# Count by type
find [input_path] -type f \( -name "*.ogg" -o -name "*.mp3" -o -name "*.m4a" -o -name "*.wav" \) | wc -l   # audio
find [input_path] -type f \( -name "*.jpg" -o -name "*.png" -o -name "*.jpeg" \) | wc -l                  # images
find [input_path] -type f \( -name "*.mp4" -o -name "*.mov" -o -name "*.mkv" \) | wc -l                   # video
find [input_path] -type f \( -name "*.json" -o -name "*.html" -o -name "*.pdf" -o -name "*.txt" \) | wc -l # text/structured
```
...
- **face-recognition-specialist**: "Batch recognize faces in [image_path]. Cluster unknown faces into identity groups. Cross-reference with known senders if the source format exposes them. Output to /sandbox/workspace/face_results.json"
...
### Phase 5: Clustering
delegate_to **content-clusterer**: "Cluster all extracted content from /sandbox/workspace/ into semantic topics. Check existing Neo4j/SurrealDB topics for reuse. Output to /sandbox/workspace/clusters.json"
...
- Identity map (people → face clusters → speaker labels → source-format senders)

## Format-specific parse steps

Different source formats need different parsers. Discover what's available:

```bash
ls /sandbox/*_parser.py 2>/dev/null
python3 /sandbox/<format>_parser.py --help
```

Typical flow for a structured-export ingest:
1. `python3 /sandbox/<format>_parser.py parse --input [export_dir] --output /sandbox/workspace/parsed_items.json`
2. `python3 /sandbox/<format>_parser.py stats --input /sandbox/workspace/parsed_items.json`
3. Text extraction on the parsed items
4. Process media files with appropriate workers
5. Cross-reference face clusters with source-format senders (if exposed by the parser) for identity resolution

For PDFs, use `INGEST_PDF_DOCUMENTS`. For raw HTML or text, use kreuzberg directly. The parser's docstring is authoritative for format-specific edge cases.
...
- Clusters are coherent and mapped to existing topics
- Knowledge graph has meaningful relationships, not just nodes
- **Embedding coverage complete** — see `AUDIT_QUALITY_GATES` Check 7. Partial vector populations silently break semantic search.
