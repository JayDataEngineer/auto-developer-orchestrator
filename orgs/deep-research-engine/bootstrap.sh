#!/usr/bin/env bash
# AUTO-GENERATED — bootstrap.sh for deep-research-engine.
# Source of truth: orgs/deep-research-engine/org.toml → [sandbox.bootstrap] + tier.
...
if ! curl -sf -o /dev/null --max-time 3 -X POST "${MEDIA_MCP_URL:-http://localhost:8101}/mcp" -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" -d "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\"}" >/dev/null 2>&1; then
  log "warning: media-mcp — multimodal ingest paths may degrade (set MEDIA_MCP_URL)"
else
  ok "media-mcp"
fi
if ! curl -sf -o /dev/null --max-time 3 -X POST "${WEB_MCP_URL:-http://localhost:8327}/mcp" -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" -d "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\"}" >/dev/null 2>&1; then
  log "warning: web-mcp — web research tools may degrade (set WEB_MCP_URL)"
else
  ok "web-mcp"
fi
