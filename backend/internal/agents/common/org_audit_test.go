...
// TestOrgSandboxEnvHasNoHostDockerInternal is the Go-side mirror of the
// host.docker.internal validator in scripts/org_build.py::_scan_host_docker_internal.
// It scans every org's GENERATED pux.yaml + docker-compose.yml for the trap
// pattern, catching drift if someone hand-edits a derived file without
// re-running `task org-build`.
//
// host.docker.internal is Docker-Desktop-only. On Linux with network_mode: host
// (which all sandbox orgs use) it does NOT resolve. Any env var or smoke-test
// string carrying it is a latent bug. The Python validator rejects it at
// source; this test is defense-in-depth.
//
// See feedback_host_docker_internal_linux_trap.md.
func TestOrgSandboxEnvHasNoHostDockerInternal(t *testing.T) {
	orgsDir := findOrgsDir(t)
	if orgsDir == "" {
		t.Skip("orgs/ directory not found")
	}
	...
	// Files that are auto-generated by `task org-build` from org.toml.
	// If any of these carry the bug, either the source org.toml has it (caught
	// by the Python validator) or someone hand-edited the derived file (caught
	// by this test). Both paths fail loud.
	derivedFiles := []string{"pux.yaml", "docker-compose.yml", "bootstrap.sh"}
	...
	for _, entry := range entries {
		...
		orgCount++
		for _, fname := range derivedFiles {
			path := filepath.Join(orgsDir, entry.Name(), fname)
			data, err := os.ReadFile(path)
			if err != nil {
				continue
			}
	...
	if len(hits) > 0 {
		var b strings.Builder
		fmt.Fprintf(&b, "%d derived files contain 'host.docker.internal' "+
			"(does not resolve on Linux host network mode):\n", len(hits))
		for _, h := range hits {
			fmt.Fprintf(&b, "  - orgs/%s/%s:%d: %s\n",
				h.orgName, h.file, h.lineNo, h.line)
		}
		b.WriteString("\nFix: change the value to 'localhost' in orgs/<org>/org.toml, " +
			"then run `task org-build <org>` to regenerate derived files.\n" +
			"See feedback_host_docker_internal_linux_trap.md.")
		t.Errorf("DRIFT:\n%s", b.String())
	}
}
...
// TestOrgBootstrapUsesPhysicalPwd enforces that every bootstrap.sh resolves
// its script dir via `pwd -P` (physical path, symlinks resolved), not bare
// `pwd`. Without -P, invoking bootstrap via the ~/.pux/orgs/<name> symlink
// labels the container with the symlink path while Pux's org resolver queries
// by the canonical path — string mismatch causes Pux to spawn a sibling
// container instead of adopting the compose one.
//
// See feedback_container_reuse_label_discovery.md +
// feedback_symlink_canonical_label_match.md.
func TestOrgBootstrapUsesPhysicalPwd(t *testing.T) {
	orgsDir := findOrgsDir(t)
	if orgsDir == "" {
		t.Skip("orgs/ directory not found")
	}
	...
	barePwd := regexp.MustCompile(`&&\s*pwd(?:\s*"|\s*$|\s*\))`)
	barePwdExceptP := regexp.MustCompile(`&&\s*pwd\s+[^-P]`)
	...
	for _, entry := range entries {
		...
		bootstrapPath := filepath.Join(orgsDir, entry.Name(), "bootstrap.sh")
		...
		// Look for the SCRIPT_DIR / ORG_DIR line: should use pwd -P.
		// Reject `pwd` or `pwd ` (without -P) but allow `pwd -P`.
		if loc := barePwd.FindStringIndex(str); loc != nil {
			// Confirm it's NOT followed by -P
			tail := str[loc[1]:]
			if !strings.HasPrefix(strings.TrimLeft(tail, " "), "-P") &&
				!strings.HasPrefix(strings.TrimLeft(tail, " "), "-") {
				drifted = append(drifted, entry.Name())
				continue
			}
		}
		if loc := barePwdExceptP.FindStringIndex(str); loc != nil {
			drifted = append(drifted, entry.Name())
		}
	}
	...
	if len(drifted) > 0 {
		t.Errorf(
			"These bootstraps use bare `pwd` instead of `pwd -P` (label path won't match Pux's canonical query → sibling container spawn):\n  - %s",
			strings.Join(drifted, "\n  - "),
		)
	}
}

// TestOrgBootstrapSpawningContainerSupportsDown enforces the symmetric-lifecycle
// contract: if a bootstrap.sh starts a container via `docker compose up`, it
// MUST also support `--down` as the inverse operation.
//
// Without this rule, operators get asymmetric drift: 6 orgs support `./bootstrap.sh --down`
// for clean teardown, then a 7th org spawns a container with no documented way to
// tear it down. The operator is forced to read the source to discover `docker compose down`
// is the manual equivalent — exactly the kind of implicit contract the org rework
// was supposed to eliminate.
//
// Host-side bootstraps (twitter-agent: cookie extraction only, no compose) are
// naturally exempt — the regex only fires when `docker compose up` is present.
//
// Grounded in PR4 (container lifecycle, 2026-06-24): bootstrap.sh --down was
// retrofit into 6 orgs + the canonical Jinja2 template, but no audit test
// pinned the contract. This test closes that gap.
func TestOrgBootstrapSpawningContainerSupportsDown(t *testing.T) {
	orgsDir := findOrgsDir(t)
	if orgsDir == "" {
		t.Skip("orgs/ directory not found")
	}
	...
	composeUpRe := regexp.MustCompile(`docker\s+compose\s+up`)
	downBranchRe := regexp.MustCompile(`--down`)
	...
	for _, entry := range entries {
		...
		str := string(body)
		if !composeUpRe.MatchString(str) {
			continue // host-side bootstrap, no container to bring down
		}
		if !downBranchRe.MatchString(str) {
			drifted = append(drifted, entry.Name())
		}
	}
	...
	if len(drifted) > 0 {
		t.Errorf(
			"These bootstraps run `docker compose up` but lack a `--down` branch — "+
				"operators have no symmetric teardown path:\n  - %s\n"+
				"Add a --down branch that runs `docker compose down` (no -v, preserves named volumes).",
			strings.Join(drifted, "\n  - "),
		)
	}
}
...
// TestBrowserCapabilityCookieToolIsGeneric enforces that the host-side
// browser-cookie extractor is a GENERIC browser-capability tool, not a
// twitter-agent-specific helper. Guards against the slow drift back to
// "one org, one cookie tool."
//
// Contracts enforced:
//
//  1. The extractor lives at orgs/_shared/sandbox/extract_browser_cookies.py
//     (reachable by every browser-using org via @shared/sandbox/).
//  2. It supports the standard browser set — chrome, brave, edge, chromium,
//     opera, vivaldi, firefox. Someone trimming the dict for "we only use
//     brave today" is the canonical DRY violation.
//  3. The browser capability SKILL.md documents the extractor + recipe so
//     org authors can wire it up without reading the script source.
//  4. The shared paths.py module exposes a generic browser_session(domain)
//     helper (domain-keyed, not site-branded) so in-sandbox code can resolve
//     session files without hardcoding paths.
//
// See config/capabilities/browser/SKILL.md "Pulling host-browser cookies"
// for the canonical recipe + [[sandbox.bootstrap.host_setup]] block.
func TestBrowserCapabilityCookieToolIsGeneric(t *testing.T) {
	orgsDir := findOrgsDir(t)
	if orgsDir == "" {
		t.Skip("orgs/ directory not found")
	}
	repoRoot := filepath.Dir(orgsDir)

	// (1) Script exists in the shared location.
	extractor := filepath.Join(orgsDir, "_shared", "sandbox", "extract_browser_cookies.py")
	body, err := os.ReadFile(extractor)
	if err != nil {
		t.Fatalf("extract_browser_cookies.py not found at shared path %s: %v\n"+
			"The cookie tool must live at orgs/_shared/sandbox/ so any "+
			"browser-using org can wire it up via @shared/sandbox/.",
			extractor, err)
	}
	extractorSrc := string(body)

	// (2) Standard browser set declared in FLATPAK_PATHS + help text.
	requiredBrowsers := []string{
		"brave", "chrome", "chromium", "edge", "opera", "vivaldi", "firefox",
	}
	var missing []string
	for _, b := range requiredBrowsers {
		// FLATPAK_PATHS covers 6 of 7 (firefox uses a different layout).
		// The help text + argparse epilog list the full set including firefox.
		if !strings.Contains(extractorSrc, fmt.Sprintf("%q", b)) &&
			!strings.Contains(extractorSrc, "\""+b+"\"") {
			missing = append(missing, b)
		}
	}
	if len(missing) > 0 {
		t.Errorf("extract_browser_cookies.py lost multi-browser support — "+
			"these browsers no longer appear in the source: %v\n"+
			"The tool must support the full standard set: %v",
			missing, requiredBrowsers)
	}

	// (3) Browser SKILL.md documents the extractor + recipe.
	skillMd := filepath.Join(repoRoot, "config", "capabilities", "browser", "SKILL.md")
	skillBody, err := os.ReadFile(skillMd)
	if err != nil {
		t.Fatalf("browser SKILL.md not found at %s: %v", skillMd, err)
	}
	skillSrc := string(skillBody)
	for _, phrase := range []string{
		"extract_browser_cookies.py",
		"Wiring it into any org",
		"[[sandbox.bootstrap.host_setup]]",
	} {
		if !strings.Contains(skillSrc, phrase) {
			t.Errorf("browser SKILL.md missing phrase %q — the cookie-tool "+
				"recipe must be documented so org authors can wire it up "+
				"without reading the script source.", phrase)
		}
	}

	// (4) paths.py exposes browser_session(domain) helper.
	pathsPy := filepath.Join(orgsDir, "_shared", "sandbox", "paths.py")
	pathsBody, err := os.ReadFile(pathsPy)
	if err != nil {
		t.Fatalf("paths.py not found at %s: %v", pathsPy, err)
	}
	pathsSrc := string(pathsBody)
	if !strings.Contains(pathsSrc, "def browser_session(") {
		t.Errorf("paths.py missing generic browser_session(domain) helper — "+
			"in-sandbox code must be able to resolve session files without "+
			"site-branded helpers like twitter_cookies().")
	}

	// (5) The script is NOT branded as twitter-only.
	for _, brand := range []string{"twitter_session", "Twitter session", "twitter-only"} {
		if strings.Contains(extractorSrc, brand) {
			t.Errorf("extract_browser_cookies.py contains %q — the tool must "+
				"be domain-generic, not branded for one org.", brand)
		}
	}
}
