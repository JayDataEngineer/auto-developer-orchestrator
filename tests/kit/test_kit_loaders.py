...
def tree(tmp_path: Path) -> Path:
    ...
    (org / "agents" / "worker.md").write_text(
        "---\nname: worker\ndescription: d\ntools: [generate_form]\nskills: [profiles/kitorg/skills]\n---\n\n# Worker\n\nDo work.\n"
    )
    (org / "skills" / "wan2gp" / "SKILL.md").write_text(
        "---\nname: wan2gp\ndescription: d\n---\n\n# Wan2GP\n\nparams.\n"
    )
    shared = tmp_path / "profiles" / "_shared" / "agents"
...
def test_load_agent_spec_local_wins_over_shared(tree: Path) -> None:
    ...
    assert worker["skills"] == ["profiles/kitorg/skills"]
...
def test_resolve_skills_local_vs_workspace(tree: Path) -> None:
    ...
    local = _resolve_skills("profiles/kitorg/skills", "worker", project_root=tree)
    assert local == [str(tree / "profiles" / "kitorg" / "skills")]

    # harness mode (workspace_root set): container-absolute paths.
    container = _resolve_skills(
        "profiles/kitorg/skills", "worker", project_root=tree, workspace_root="/sandbox/workspace",
    )
    assert container == ["/sandbox/workspace/profiles/kitorg/skills"]


def test_resolve_skills_rejects_bad_paths(tree: Path) -> None:
    with pytest.raises(KeyError, match="no such directory"):
        _resolve_skills("profiles/kitorg/nope", "worker", project_root=tree)
...
def test_merge_extends_skills_add_union() -> None:
    merged = _merge_extends(_base_spec(), {"skills_add": ["profiles/o/skills"]}, "")
    assert merged["skills"] == ["profiles/shared/skills", "profiles/o/skills"]


def test_merge_extends_skills_explicit_full_replace() -> None:
    merged = _merge_extends(_base_spec(), {"skills": ["profiles/only/skills"]}, "")
    assert merged["skills"] == ["profiles/only/skills"]
...
def test_merge_extends_declared_keys_never_vanish() -> None:
    """The delta's non-core declarations (``middleware`` / ``rubric`` / ``mcp``
    / any future key) MUST survive the merge, delta wins — a declared key
    silently vanishing from the merged spec is the forbidden drop."""
    merged = _merge_extends(_base_spec(), {
        "middleware": ["rubric"],
        "rubric": "grade hard",
        "mcp": ["sandbox_browser"],
        "capabilities": [{"kind": "mcp", "ref": "sandbox_browser"}],
    }, "DELTA BODY")
    assert merged["middleware"] == ["rubric"]
    assert merged["rubric"] == "grade hard"
    assert merged["mcp"] == ["sandbox_browser"]
    # base-only keys still pass through, merge sugar still consumed
    assert merged["name"] == "base"
    assert merged["tools"] == ["alpha", "beta"]
    assert merged["system_prompt"] == "BASE BODY\n\nDELTA BODY"
    # an unset delta key inherits the base's
    assert _merge_extends(_base_spec(), {"tools": ["delta"]}, "")["skills"] == ["profiles/shared/skills"]
...
def _write_agent(root: Path, slug: str, *, org: str = "_shared",
    ...
    """Write ``profiles/<org>/agents/<slug>.md`` with optional extra frontmatter
    lines (used for ``extends:`` + the delta fields)."""
    d = root / "profiles" / org / "agents"
