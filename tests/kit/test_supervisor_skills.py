...
``profiles/_shared/skills`` + THIS org's own ``skills/`` (existing dirs only),
...
from profiles.loaders import supervisor_skills_roots
...
def test_returns_shared_plus_own_host_absolute(tmp_path: Path):
    ...
    _make_skills(tmp_path, "profiles/_shared/skills")
    _make_skills(tmp_path, "profiles/foo/skills")
    out = supervisor_skills_roots("foo", tmp_path)
    assert out == [
        str(tmp_path / "profiles/_shared/skills"),
        str(tmp_path / "profiles/foo/skills"),
    ]
...
def test_returns_container_absolute_when_workspace_root_pinned(tmp_path: Path):
    ...
    assert out == [
        "/sandbox/workspace/profiles/_shared/skills",
        "/sandbox/workspace/profiles/foo/skills",
    ]
...
def test_drops_missing_dirs_existing_only(tmp_path: Path):
    ...
    _make_skills(tmp_path, "profiles/_shared/skills")  # no profiles/foo/skills
    out = supervisor_skills_roots("foo", tmp_path)
    assert out == [str(tmp_path / "profiles/_shared/skills")]


def test_specialist_org_uses_specialists_path(tmp_path: Path):
    """A specialist org (e.g. ``coder``) keeps its skills under
    ``profiles/specialists/<org>/skills``; the bare ``profiles/<org>/skills``
    candidate is correctly absent for it."""
    _make_skills(tmp_path, "profiles/_shared/skills")
    _make_skills(tmp_path, "profiles/specialists/coder/skills")
    out = supervisor_skills_roots("coder", tmp_path, workspace_root="/sandbox/workspace")
    assert out == [
        "/sandbox/workspace/profiles/_shared/skills",
        "/sandbox/workspace/profiles/specialists/coder/skills",
    ]
...
def test_candidate_order_is_shared_then_own(tmp_path: Path):
    ...
    _make_skills(tmp_path, "profiles/_shared/skills")
    out = supervisor_skills_roots("foo", tmp_path)
    assert out[0].endswith("profiles/_shared/skills")
    assert out[1].endswith("profiles/foo/skills")
