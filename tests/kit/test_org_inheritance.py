...
The portable, project_root-parameterized core (``profiles.loaders``): NO
...
def _make_org(
    ...
    """Write ``profiles/<name>/AGENTS.md`` + (when there's a roster or an extends)
...
def _make_agent(
    ...
    """Write ``profiles/<org>/agents/<slug>.md`` (frontmatter + body)."""
    agents_dir = root / "profiles" / org / "agents"
...
def test_chain_parent_without_agents_md_raises(tmp_path: Path) -> None:
    ...
    (tmp_path / "profiles" / "naked").mkdir(parents=True)
...
def test_roster_inherit_roster_false_prunes_parent(tmp_path: Path) -> None:
    ...
    _make_org(tmp_path, "base", body="# Base\n", agents=["alpha", "beta"])
    _make_org(tmp_path, "child", body="# Child\n", agents=["gamma"], extends="base")
    (tmp_path / "profiles" / "child" / "org.yaml").write_text(
...
def test_search_dirs_child_first_then_ancestors_then_shared(tmp_path: Path) -> None:
    ...
    (tmp_path / "profiles" / "_shared" / "agents").mkdir(parents=True)
    dirs = [str(d) for d in _agent_search_dirs("child", tmp_path)]
    assert dirs[0].endswith("profiles/child/agents")
    assert dirs[1].endswith("profiles/base/agents")
    assert dirs[-1].endswith("profiles/_shared/agents")
