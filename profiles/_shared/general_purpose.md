...
documentation: |
  ...
  folded workspace — the ``general_purpose_subagent:`` profile seam died with
  the harness. Retained as the canonical text if the seam returns. No Python
  constants — the file is the single source.
...
This file was loaded by the pre-fold harness when an org enabled the
`general-purpose` subagent via `profile.yaml`'s native
