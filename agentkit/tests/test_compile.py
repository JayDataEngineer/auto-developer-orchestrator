...
from pux_agentkit import compile_org, load_subagents
...
def test_import_isolation_no_docker_no_harness() -> None:
    """LOAD-BEARING isolation contract: importing the kit pulls in NEITHER
    ``docker`` NOR ``pux_harness``. Run in a fresh subprocess so prior test
    imports don't pollute ``sys.modules``."""
    code = (
        "import sys; import pux_agentkit; import pux_agentkit.compile; "
        "import pux_agentkit.loaders; "
        "print(repr(('docker' in sys.modules, 'pux_harness' in sys.modules)))"
    )
    out = subprocess.check_output([sys.executable, "-c", code], text=True).strip()
    docker_loaded, harness_loaded = eval(out)
    assert not docker_loaded, "kit must not import docker"
    assert not harness_loaded, "kit must not import pux_harness"
