...
async def lifespan(app: FastAPI):
    PUX_API_DB.parent.mkdir(parents=True, exist_ok=True)
    async with AsyncSqliteSaver.from_conn_string(str(PUX_API_DB)) as saver:
        await saver.setup()
        db = await aiosqlite.connect(str(PUX_API_DB))
...
async def thread_create(body: ThreadCreate) -> dict[str, Any]:
    ...
    await app.state.db.execute(
        "INSERT INTO pux_threads (thread_id, org, metadata, created_at) VALUES (?,?,?,?)",
        (thread_id, body.agent_id, meta, _now()),
    )
...
async def run_ephemeral(body: EphemeralRun) -> dict[str, Any]:
    ...
    await app.state.db.execute(
        "INSERT INTO pux_threads (thread_id, org, metadata, created_at) VALUES (?,?,?,?)",
        (thread_id, body.agent_id, json.dumps(body.metadata), _now()),
    )
    await app.state.db.commit()
    try:
