...
class _SearchArgs(BaseModel):
    query: str = Field(
        ...,
        description=(
            "A distinctive phrase to find across stashed tool results AND captured "
            "events. BM25-ranked; returns handles + a snippet each."
        ),
    )
...
def _format_hits(hits: list[SearchHit]) -> str:
    if not hits:
        return "no stashed result or event matched that query."
    lines = [f"{len(hits)} hit(s):"]
    for h in hits:
        tag = f"[{h.kind}]"  # [blob] (recallable) or [event]
...
def build_context_tools(store: EventStore | None = None) -> list[StructuredTool]:
    ...
    search = StructuredTool.from_function(
        ...
        name="ctx_search",
        description=(
            "BM25 search across every offloaded tool result AND captured event for a "
            "phrase; returns matching handles + a snippet each. Use ctx_recall to pull "
            "a full blob. Use when you remember a detail but not which call produced it."
        ),
