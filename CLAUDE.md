...
specialists are split — `python`/skills/`describe_image`/browser_* are **native
Python StructuredTools** (Phase 8b–8e), desktop_* still come from the Go bridge
until 8f:
...
| 8 | Re-host sandbox in Python (`execute()`→docker exec; 13 specialist tools); wire policy enforcement here; delete Go MCP | **8a/8b/8c/8d/8e SHIPPED 2026-07-03** — **8a** `docker_exec.py` `DockerExecClient` (container discovered by `openshell.project-path` label; `exec()` via Docker SDK `exec_run(tty=False)`; `PuxSandboxBackend.execute()`/`id` retargeted from MCP `bash` to direct docker exec). **8b/8c** `native_tools.py` ports `python` (docker exec `python3 -c`), `list_skills`/`load_skill` (host FS `.pi/skills/`) — fixes a latent Go bug (skills package read `<root>/skills/` → `count:0`; Python reads the real path → `count:1`). **8d** ports `describe_image` (exit-code dispatch: 0/1/2=unavailable/3=deps + timeout); `docker_exec.exec(timeout=N)` enforces per-call deadlines via thread `.result(timeout=)` → `ExecTimeout`. **8e** ports the 5 `browser_*` tools (`_sb_post` helper builds the exact `curl -s -S --max-time N -X POST …/sb_server` cmd Go's `postJSON` did; response parsed + re-emitted via `_result`). `_result` now uses `sort_keys=True` so output is byte-equivalent to the Go bridge's `json.MarshalIndent(v,"","  ")` (Go sorts map keys — verified the live list_skills emits `count` before `skills`). **Proven E2E** (real sb_server + live glm-5.2 trees): browser_navigate example.com→title "Example Domain"+SoM element_map; browser_screenshot/evaluate round-trip; describe_image→unavailable (model absent); merge probe = 9 native + 4 bridge, no dupes. pytest 102/102, contract exit 0 (13 specialists live). Sub-steps 8f–8i remain (desktop, lifecycle, policy enforcement, then delete Go). |
