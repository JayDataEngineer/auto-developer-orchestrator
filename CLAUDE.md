...
specialists are still **`pux_sandbox_*`** MCP tools from the Go bridge (Phase
8b–8f ports them to direct docker exec too):
...
| 8 | Re-host sandbox in Python (`execute()`→docker exec; 13 specialist tools); wire policy enforcement here; delete Go MCP | **8a SHIPPED 2026-07-03** — `harness/pux_harness/docker_exec.py` (`DockerExecClient`: container discovered by `openshell.project-path` label; `exec()` via Docker SDK `exec_run(tty=False)` — SDK parses Docker framing, cleaner than the Go binary's raw-`io.Copy`+`tty=True` trick). `PuxSandboxBackend.execute()`/`id` retargeted from the Go MCP `bash` tool to direct docker exec; the MCP-envelope unwrap (`_content_text`/`_unwrap`) is gone. `graph.py` gains `shared_exec()`; backend no longer takes the MCP client (specialists still do, via `get_pux_tools`). Proven: `--check` smoke (`execute`→`('pux-ok\n',0)`, inherited `ls`→36 entries via docker exec) + E2E `pux direct --org general` glm-5.2 → researcher→native `execute`→**9** Go files (correct ground truth); the `find` calls appear in `backend.execute_log` (the docker-exec path) with zero `pux_sandbox` fs/shell leakage. pytest 102/102, contract exit 0. Sub-steps 8b–8i remain (port 13 specialists, lifecycle, policy enforcement, then delete Go). |
