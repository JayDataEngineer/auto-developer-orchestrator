...
`docker exec`** into the container — Phase 8a cut the MCP→Go middleman);
specialists are split — `python`/skills/`describe_image` are **native Python
StructuredTools** (Phase 8b–8d), browser/desktop still come from the Go bridge
until 8e/8f:
...
| `browser_navigate` / `_click` / `_type` / `_screenshot` / `_evaluate` | MCP (8e) | `curl` to in-sandbox `sb_server.py` |
| `desktop_screenshot` / `_click` / `_type` / `_key` | MCP (8f) | `xdotool` + `/usr/local/bin/desktop_observe.py` |
...
| 8 | Re-host sandbox in Python (`execute()`→docker exec; 13 specialist tools); wire policy enforcement here; delete Go MCP | **8a/8b/8c/8d SHIPPED 2026-07-03** — **8a** `docker_exec.py` `DockerExecClient` (container discovered by `openshell.project-path` label; `exec()` via Docker SDK `exec_run(tty=False)`; `PuxSandboxBackend.execute()`/`id` retargeted from MCP `bash` to direct docker exec). **8b/8c** `native_tools.py` ports `python` (docker exec `python3 -c`), `list_skills`/`load_skill` (host FS `.pi/skills/`) to native StructuredTools — fixes a latent Go bug (skills package read `<root>/skills/` → `count:0`; Python reads the real path → `count:1`). **8d** ports `describe_image` (faithful exit-code dispatch: 0=success, 1=inference_failed, 2=unavailable, 3=deps_missing, +timeout→`reason:"timeout"`); `docker_exec.exec(timeout=N)` now enforces per-call deadlines via a thread-based `.result(timeout=)` wrapper raising `ExecTimeout` (the documented follow-up, fulfilled). `graph.py` merges `native(PORTED_SPECIALISTS) + bridge(SPECIALIST_TOOLS - PORTED)`; `bridge.py` exports `SPECIALIST_TOOLS` for the subtraction. **Proven E2E** (each live glm-5.2 tree): python→`{success,output:'2\n'}`, list_skills→count 1, describe_image→`unavailable` (model absent); merge probe = 4 native + 9 bridge, no dupes; exec timeout fires on `sleep 3`/timeout=1. pytest 102/102, contract exit 0 (13 specialists live). Sub-steps 8e–8i remain (browser/desktop, lifecycle, policy enforcement, then delete Go). |
